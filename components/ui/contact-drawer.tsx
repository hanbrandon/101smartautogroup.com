...
import { X, Send, Loader2, CheckCircle2, Car, BadgeDollarSign, Info } from 'lucide-react';
...
export const ContactDrawer = ({ isOpen, onClose }: ContactDrawerProps) => {
  const [activeTab, setActiveTab] = useState<'new_car' | 'sell_car'>('new_car');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = activeTab === 'new_car' 
      ? { make: formData.make, model: formData.model, purchaseType: formData.purchaseType }
      : { 
          vin: formData.vin, 
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          ...
          setFormData({
            ...
            make: '', model: '', purchaseType: 'lease',
            vin: '', mileage: '', exteriorColor: '', accidentHistory: 'No Accidents', financialStatus: 'Finance / Loan'
          });
        }, 2000);
      } else {
        setStatus('error');
        ...
        <>
          ...
          >
            <div className="p-8 border-b border-white/5 flex justify-between items-center">
              <h2 className="text-2xl font-bold tracking-tighter italic">Get in touch</h2>
                  ...
                  <div className="flex gap-2 p-1.5 bg-white/5 rounded-[24px] border border-white/5">
                    <button
                      ...
                      className={cn(
                        ...
                        activeTab === 'new_car' ? "bg-white text-black shadow-xl" : "text-white/40 hover:text-white hover:bg-white/5"
                      )}
                    >
                      <Car size={18} />
                      New Car
                    ...
                    <button
                      ...
                      className={cn(
                        "flex-1 flex flex-col items-center justify-center gap-2 py-4 rounded-[20px] text-[10px] font-bold uppercase tracking-widest transition-all border border-transparent",
                        activeTab === 'sell_car' ? "bg-white text-black shadow-xl" : "text-white/40 hover:text-white hover:bg-white/5"
                      )}
                    >
                    ...
                    {activeTab === 'new_car' && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <select value={formData.purchaseType} onChange={e => setFormData({...formData, purchaseType: e.target.value})} className={selectStyles}>
                      ...
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <input required value={formData.vin} onChange={e => setFormData({...formData, vin: e.target.value})} className={inputStyles} placeholder="Vehicle Identification Number" />
                          </div>
                          <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            ...
                            <select value={formData.accidentHistory} onChange={e => setFormData({...formData, accidentHistory: e.target.value})} className={selectStyles}>
                          ...
                          <div className="space-y-2">
                            ...
                            <select value={formData.financialStatus} onChange={e => setFormData({...formData, financialStatus: e.target.value})} className={selectStyles}>
